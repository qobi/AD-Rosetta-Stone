...
if __name__=="__main__":
	for i in range(10):
	        ...
	        eta = 0.1
	        for j in range(1000):
	                probability =  objective(p)
		        probability.backward()
		        p0_record = (p[0]+eta*p[0].grad).data.numpy()
		        p1_record = (p[1]+eta*p[1].grad).data.numpy()
		        p[0] = torch.tensor(p0_record, requires_grad = True)
		        p[1] = torch.tensor(p1_record, requires_grad = True)
                print p[0], p[1]
